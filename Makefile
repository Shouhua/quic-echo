...
run_client: client
	@SSLKEYLOGFILE="${PROJECT_ROOT_DIR}/keylog.txt" ./build/client "localhost" "4433" "${CA}"

debug_client: client
	@SSLKEYLOGFILE="${PROJECT_ROOT_DIR}/keylog.txt" gdb --args ./build/client "localhost" "4433" "${CA}"
...
run_server: server
	@./build/server localhost 4433 "${SERVER_CERT}" "${SERVER_KEY}"
	
debug_server: server
	@gdb --args ./build/server localhost 4433 "${SERVER_CERT}" "${SERVER_KEY}"
